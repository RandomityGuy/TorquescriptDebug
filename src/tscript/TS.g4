...
object_decl
   ...
   | RW_DECLARE class_name_expr '(' '[' object_name ']' parent_block object_args ')' '{' object_declare_block '}'
   | RW_DECLARE class_name_expr '(' '[' object_name ']' parent_block object_args ')'
   | RW_DECLARESINGLETON class_name_expr '(' object_name parent_block object_args ')' '{' object_declare_block '}'
   | RW_DECLARESINGLETON class_name_expr '(' object_name parent_block object_args ')'
   ;
...
while_stmt
   ...
   | RW_DO stmt_block RW_WHILE '(' expr ')'
   ;

for_stmt
   : RW_FOR '(' expr ';' expr ';' expr ')' stmt_block
   | RW_FOR '(' expr ';' expr ';'      ')' stmt_block
   | RW_FOR '(' expr ';'      ';' expr ')' stmt_block
   | RW_FOR '(' expr ';'      ';'      ')' stmt_block
   | RW_FOR '('      ';' expr ';' expr ')' stmt_block
   | RW_FOR '('      ';' expr ';'      ')' stmt_block
   | RW_FOR '('      ';'      ';' expr ')' stmt_block
   | RW_FOR '('      ';'      ';'      ')' stmt_block
...
foreach_stmt
   : RW_FOREACH '(' expr RW_IN expr ')' stmt_block
   | RW_FOREACHSTR '(' expr RW_IN expr ')' stmt_block
   ;

assert_expr
   : RW_ASSERT '(' expr ')'
   | RW_ASSERT '(' expr ',' STRATOM ')'
   ;
...
expr
   ...
   | expr OP_INTNAME class_name_expr
   | expr OP_INTNAMER class_name_expr
   | IDENT
   | STRATOM
   | VAR
   | VAR '[' aidx_expr ']'
...
RW_FOR : 'for' ;
RW_FOREACH : 'foreach' ;
RW_FOREACHSTR : 'foreach$' ;
RW_CONTINUE : 'continue' ;
RW_DEFINE : 'function' ;
RW_DECLARE : 'new' ;
RW_DATABLOCK : 'datablock' ;
RW_DECLARESINGLETON : 'singleton' ;
RW_CASE : 'case' ;
RW_SWITCHSTR : 'switch$' ;
RW_SWITCH : 'switch' ;
RW_DEFAULT : 'default' ;
