...
class CompileContext {
	constructor(codeSize,lineBreakPairSize) {
		this.codeStream = new Array(codeSize);
		this.lineBreakPairs = new Array(lineBreakPairSize);
...
class VarCollector {
	...
	visitInternalNameAccessExpr(expr) {
	}
...
class Parser {
		...
		if(this.match([TokenType.Dollar,TokenType.Modulus])) {
			...
			if(this.match(varStart)) {
				...
				while(this.match(varMid)) {
					...
					let tmp = tok.type._hx_index == 70 ? "::" : tok.literal;
	...
	foreachStmt() {
		if(this.match([TokenType.Foreach,TokenType.ForeachStr])) {
			let foreachLine = this.peek().line;
			let isString = this.match([TokenType.ForeachStr]);
			this.advance();
			this.consume(TokenType.LParen,"Expected '(' after 'foreach'");
			let init = this.expression();
			this.consume(TokenType.In,"Expected 'in' after foreach variable");
			let container = this.expression();
			this.consumeSynchronize(TokenType.RParen,"Expected ')' after foreach container");
			let body = [];
			let islist = false;
			if(this.match([TokenType.LBracket])) {
				this.advance();
			...
			let iterstmt = new expr_IterStmt(foreachLine,init,container,body);
			iterstmt.isString = isString;
			return iterstmt;
		} else {
			return null;
		}
	...
	whileStmt() {
		if(this.match([TokenType.While])) {
			...
			this.consume(TokenType.While,"Expected 'while' after do loop body");
			this.consume(TokenType.LParen,"Expected '(' after 'while'");
			let condExpr = this.expression();
			this.consumeSynchronize(TokenType.RParen,"Expected ')' after condition in while loop");
			let loopstmt = new expr_LoopStmt(doLine,condExpr,null,null,body);
			loopstmt.isStatementList = islist;
			loopstmt.isDoWhile = true;
			return loopstmt;
		} else {
			return null;
		}
	...
	ifStmt() {
		if(this.match([TokenType.If])) {
			...
			let ifstmt = new expr_IfStmt(ifLine,condExpr,body,elseBody);
			ifstmt.ifStmtList = bodylist;
			ifstmt.elseStmtList = elselist;
			return ifstmt;
		} else {
			return null;
		}
	...
	stmtExpr() {
		...
		if(expr != null) {
			if(this.match([TokenType.Dot])) {
				...
				case 26:case 27:case 28:case 29:case 30:case 31:case 32:case 33:case 35:
				...
				case 65:
					...
					break;
				case 81:case 82:
					let pexpr = new expr_ParenthesisExpr(new expr_SlotAssignOpExpr(expr,arrAccess,labelAccess,null,nextTok));
					pexpr.implicit = true;
					return pexpr;
				default:
					this.current = curPos;
				...
				case 81:case 82:
					let pexpr = new expr_ParenthesisExpr(new expr_AssignOpExpr(varExpr,null,nextTok));
					pexpr.implicit = true;
					return pexpr;
				default:
					this.current = curPos;
	...
	assertStmt() {
		if(this.match([TokenType.Assert])) {
			...
			if(this.match([TokenType.Comma])) {
				this.advance();
				msg = this.consume(TokenType.String,"Expected string after assert expression");
			}
			this.consumeSynchronize(TokenType.RParen,"Expected ')' after assert expression");
			this.consume(TokenType.Semicolon,"Expected ';' after assert expression");
			return new expr_AssertStmt(line,expr,msg);
		} else {
			return null;
		}
	}
	objectDecl() {
		if(this.match([TokenType.New,TokenType.Singleton])) {
			let tok = this.advance();
			...
			let sqBrackets = false;
			if(this.match([TokenType.LeftSquareBracket]) && tok.type == TokenType.New) {
				this.advance();
				sqBrackets = true;
			}
			...
			if(!this.match([TokenType.RParen])) {
				...
				if(sqBrackets) {
					this.consume(TokenType.RightSquareBracket,"Expected ']' after object name");
				}
				if(this.match([TokenType.Colon])) {
					this.advance();
			...
			if(this.match([TokenType.LBracket])) {
				...
				let odcl = new expr_ObjectDeclExpr(classNameExpr,parentObj,objNameExpr,objArgs,slotAssigns,subObjects,false);
				odcl.singleton = tok.type == TokenType.Singleton;
				return odcl;
			} else {
				let odcl = new expr_ObjectDeclExpr(classNameExpr,parentObj,objNameExpr,objArgs,[],[],false);
				odcl.singleton = tok.type == TokenType.Singleton;
				return odcl;
			}
		} else {
		...
		chainExpr = function() {
			...
			while(_gthis.match([TokenType.Dot])) {
				...
				if(_gthis.match([TokenType.LParen])) {
					...
					case 81:case 82:
						_gthis.advance();
						let pexpr = new expr_ParenthesisExpr(new expr_SlotAssignOpExpr(chExpr,arrAccess,label,null,nextTok));
						pexpr.implicit = true;
						chExpr = pexpr;
						break;
					default:
			...
			if(_gthis.match([TokenType.IntName,TokenType.IntNameR])) {
				let tok = _gthis.advance();
				let classNameExpr = null;
				if(_gthis.match([TokenType.LParen])) {
					_gthis.advance();
					classNameExpr = new expr_ParenthesisExpr(_gthis.expression());
					_gthis.consumeSynchronize(TokenType.RParen,"Expected ')' after class name");
				} else {
					let cname = _gthis.consume(TokenType.Label,"Expected class name");
					classNameExpr = new expr_ConstantExpr(cname);
				}
				let inameEx = new expr_InternalNameAccessExpr(chExpr,classNameExpr);
				inameEx.recursive = tok.type == TokenType.IntNameR;
				chExpr = inameEx;
			}
			return chExpr;
		};
...
class Scanner {
	constructor(s) {
		...
		_g.h["singleton"] = TokenType.Singleton;
		...
		_g.h["foreach"] = TokenType.Foreach;
		_g.h["foreach$"] = TokenType.ForeachStr;
		...
		_g.h["do"] = TokenType.Do;
		_g.h["or"] = TokenType.Or;
		_g.h["in"] = TokenType.In;
		_g.h["assert"] = TokenType.Assert;
		this.keywords = _g;
		this.line = 1;
		this.current = 0;
		this.start = 0;
	...
	scanToken() {
		...
		case 45:
			if(this.match(">")) {
				this.addToken(TokenType.IntName);
			} else if(this.match("-")) {
				if(this.match(">")) {
					this.addToken(TokenType.IntNameR);
				} else {
					this.addToken(TokenType.MinusMinus);
				}
			} else {
				this.addToken(this.match("=") ? TokenType.MinusAssign : TokenType.Minus);
			}
			break;
		case 46:
			let next = this.peek();
		...
		case 47:
			if(this.match("/")) {
				...
				while(this.peek() != 42 || this.peekNext() != 47) {
					if(this.isAtEnd()) {
						console.log("src/Scanner.hx:120:","Unterminated comment.");
					}
					this.advance();
				}
		...
		default:
			...
			if(49 <= x2 && x2 <= 57) {
				...
				console.log("src/Scanner.hx:187:","Unexpected character " + this.line + " - " + c);
			}
		}
	}
	advance() {
	...
	string(delimiter,tokenType) {
		...
		if(this.isAtEnd()) {
			console.log("src/Scanner.hx:246:","Unterminated string");
			return;
		}
		this.advance();
	...
	hexNumber() {
		...
		let hex = this.source.substring(this.start,this.current);
		this.addToken(TokenType.Int,"0x" + hex);
	}
	number() {
...
var TokenType = $hxEnums["TokenType"] = { __ename__:true,__constructs__:null
	...
	,Singleton: {_hx_name:"Singleton",_hx_index:1,__enum__:"TokenType",toString:$estr}
	,Package: {_hx_name:"Package",_hx_index:2,__enum__:"TokenType",toString:$estr}
	,Function: {_hx_name:"Function",_hx_index:3,__enum__:"TokenType",toString:$estr}
	,Assert: {_hx_name:"Assert",_hx_index:4,__enum__:"TokenType",toString:$estr}
	,If: {_hx_name:"If",_hx_index:5,__enum__:"TokenType",toString:$estr}
	,Else: {_hx_name:"Else",_hx_index:6,__enum__:"TokenType",toString:$estr}
	,Switch: {_hx_name:"Switch",_hx_index:7,__enum__:"TokenType",toString:$estr}
	,Case: {_hx_name:"Case",_hx_index:8,__enum__:"TokenType",toString:$estr}
	,Return: {_hx_name:"Return",_hx_index:9,__enum__:"TokenType",toString:$estr}
	,Break: {_hx_name:"Break",_hx_index:10,__enum__:"TokenType",toString:$estr}
	,New: {_hx_name:"New",_hx_index:11,__enum__:"TokenType",toString:$estr}
	,Do: {_hx_name:"Do",_hx_index:12,__enum__:"TokenType",toString:$estr}
	,While: {_hx_name:"While",_hx_index:13,__enum__:"TokenType",toString:$estr}
	,For: {_hx_name:"For",_hx_index:14,__enum__:"TokenType",toString:$estr}
	,Foreach: {_hx_name:"Foreach",_hx_index:15,__enum__:"TokenType",toString:$estr}
	,ForeachStr: {_hx_name:"ForeachStr",_hx_index:16,__enum__:"TokenType",toString:$estr}
	,True: {_hx_name:"True",_hx_index:17,__enum__:"TokenType",toString:$estr}
	,False: {_hx_name:"False",_hx_index:18,__enum__:"TokenType",toString:$estr}
	,Default: {_hx_name:"Default",_hx_index:19,__enum__:"TokenType",toString:$estr}
	,Plus: {_hx_name:"Plus",_hx_index:20,__enum__:"TokenType",toString:$estr}
	,Minus: {_hx_name:"Minus",_hx_index:21,__enum__:"TokenType",toString:$estr}
	,Multiply: {_hx_name:"Multiply",_hx_index:22,__enum__:"TokenType",toString:$estr}
	,Divide: {_hx_name:"Divide",_hx_index:23,__enum__:"TokenType",toString:$estr}
	,Modulus: {_hx_name:"Modulus",_hx_index:24,__enum__:"TokenType",toString:$estr}
	,Assign: {_hx_name:"Assign",_hx_index:25,__enum__:"TokenType",toString:$estr}
	,PlusAssign: {_hx_name:"PlusAssign",_hx_index:26,__enum__:"TokenType",toString:$estr}
	,MinusAssign: {_hx_name:"MinusAssign",_hx_index:27,__enum__:"TokenType",toString:$estr}
	,MultiplyAssign: {_hx_name:"MultiplyAssign",_hx_index:28,__enum__:"TokenType",toString:$estr}
	,OrAssign: {_hx_name:"OrAssign",_hx_index:29,__enum__:"TokenType",toString:$estr}
	,AndAssign: {_hx_name:"AndAssign",_hx_index:30,__enum__:"TokenType",toString:$estr}
	,XorAssign: {_hx_name:"XorAssign",_hx_index:31,__enum__:"TokenType",toString:$estr}
	,ModulusAssign: {_hx_name:"ModulusAssign",_hx_index:32,__enum__:"TokenType",toString:$estr}
	,DivideAssign: {_hx_name:"DivideAssign",_hx_index:33,__enum__:"TokenType",toString:$estr}
	,ShiftLeftAssign: {_hx_name:"ShiftLeftAssign",_hx_index:34,__enum__:"TokenType",toString:$estr}
	,ShiftRightAssign: {_hx_name:"ShiftRightAssign",_hx_index:35,__enum__:"TokenType",toString:$estr}
	,LessThan: {_hx_name:"LessThan",_hx_index:36,__enum__:"TokenType",toString:$estr}
	,GreaterThan: {_hx_name:"GreaterThan",_hx_index:37,__enum__:"TokenType",toString:$estr}
	,LessThanEqual: {_hx_name:"LessThanEqual",_hx_index:38,__enum__:"TokenType",toString:$estr}
	,GreaterThanEqual: {_hx_name:"GreaterThanEqual",_hx_index:39,__enum__:"TokenType",toString:$estr}
	,Not: {_hx_name:"Not",_hx_index:40,__enum__:"TokenType",toString:$estr}
	,NotEqual: {_hx_name:"NotEqual",_hx_index:41,__enum__:"TokenType",toString:$estr}
	,Equal: {_hx_name:"Equal",_hx_index:42,__enum__:"TokenType",toString:$estr}
	,Tilde: {_hx_name:"Tilde",_hx_index:43,__enum__:"TokenType",toString:$estr}
	,StringEquals: {_hx_name:"StringEquals",_hx_index:44,__enum__:"TokenType",toString:$estr}
	,StringNotEquals: {_hx_name:"StringNotEquals",_hx_index:45,__enum__:"TokenType",toString:$estr}
	,Concat: {_hx_name:"Concat",_hx_index:46,__enum__:"TokenType",toString:$estr}
	,SpaceConcat: {_hx_name:"SpaceConcat",_hx_index:47,__enum__:"TokenType",toString:$estr}
	,TabConcat: {_hx_name:"TabConcat",_hx_index:48,__enum__:"TokenType",toString:$estr}
	,NewlineConcat: {_hx_name:"NewlineConcat",_hx_index:49,__enum__:"TokenType",toString:$estr}
	,Continue: {_hx_name:"Continue",_hx_index:50,__enum__:"TokenType",toString:$estr}
	,LogicalAnd: {_hx_name:"LogicalAnd",_hx_index:51,__enum__:"TokenType",toString:$estr}
	,LogicalOr: {_hx_name:"LogicalOr",_hx_index:52,__enum__:"TokenType",toString:$estr}
	,LeftBitShift: {_hx_name:"LeftBitShift",_hx_index:53,__enum__:"TokenType",toString:$estr}
	,RightBitShift: {_hx_name:"RightBitShift",_hx_index:54,__enum__:"TokenType",toString:$estr}
	,BitwiseAnd: {_hx_name:"BitwiseAnd",_hx_index:55,__enum__:"TokenType",toString:$estr}
	,BitwiseOr: {_hx_name:"BitwiseOr",_hx_index:56,__enum__:"TokenType",toString:$estr}
	,BitwiseXor: {_hx_name:"BitwiseXor",_hx_index:57,__enum__:"TokenType",toString:$estr}
	,Label: {_hx_name:"Label",_hx_index:58,__enum__:"TokenType",toString:$estr}
	,Int: {_hx_name:"Int",_hx_index:59,__enum__:"TokenType",toString:$estr}
	,String: {_hx_name:"String",_hx_index:60,__enum__:"TokenType",toString:$estr}
	,TaggedString: {_hx_name:"TaggedString",_hx_index:61,__enum__:"TokenType",toString:$estr}
	,Exp: {_hx_name:"Exp",_hx_index:62,__enum__:"TokenType",toString:$estr}
	,Float: {_hx_name:"Float",_hx_index:63,__enum__:"TokenType",toString:$estr}
	,Ws: {_hx_name:"Ws",_hx_index:64,__enum__:"TokenType",toString:$estr}
	,LParen: {_hx_name:"LParen",_hx_index:65,__enum__:"TokenType",toString:$estr}
	,Colon: {_hx_name:"Colon",_hx_index:66,__enum__:"TokenType",toString:$estr}
	,RParen: {_hx_name:"RParen",_hx_index:67,__enum__:"TokenType",toString:$estr}
	,LBracket: {_hx_name:"LBracket",_hx_index:68,__enum__:"TokenType",toString:$estr}
	,RBracket: {_hx_name:"RBracket",_hx_index:69,__enum__:"TokenType",toString:$estr}
	,DoubleColon: {_hx_name:"DoubleColon",_hx_index:70,__enum__:"TokenType",toString:$estr}
	,Comma: {_hx_name:"Comma",_hx_index:71,__enum__:"TokenType",toString:$estr}
	,Semicolon: {_hx_name:"Semicolon",_hx_index:72,__enum__:"TokenType",toString:$estr}
	,LeftSquareBracket: {_hx_name:"LeftSquareBracket",_hx_index:73,__enum__:"TokenType",toString:$estr}
	,RightSquareBracket: {_hx_name:"RightSquareBracket",_hx_index:74,__enum__:"TokenType",toString:$estr}
	,Or: {_hx_name:"Or",_hx_index:75,__enum__:"TokenType",toString:$estr}
	,In: {_hx_name:"In",_hx_index:76,__enum__:"TokenType",toString:$estr}
	,Dollar: {_hx_name:"Dollar",_hx_index:77,__enum__:"TokenType",toString:$estr}
	,Dot: {_hx_name:"Dot",_hx_index:78,__enum__:"TokenType",toString:$estr}
	,IntName: {_hx_name:"IntName",_hx_index:79,__enum__:"TokenType",toString:$estr}
	,IntNameR: {_hx_name:"IntNameR",_hx_index:80,__enum__:"TokenType",toString:$estr}
	,PlusPlus: {_hx_name:"PlusPlus",_hx_index:81,__enum__:"TokenType",toString:$estr}
	,MinusMinus: {_hx_name:"MinusMinus",_hx_index:82,__enum__:"TokenType",toString:$estr}
	,QuestionMark: {_hx_name:"QuestionMark",_hx_index:83,__enum__:"TokenType",toString:$estr}
	,Eof: {_hx_name:"Eof",_hx_index:84,__enum__:"TokenType",toString:$estr}
	,Comment: ($_=function(multiline) { return {_hx_index:85,multiline:multiline,__enum__:"TokenType",toString:$estr}; },$_._hx_name="Comment",$_.__params__ = ["multiline"],$_)
	,Unknown: {_hx_name:"Unknown",_hx_index:86,__enum__:"TokenType",toString:$estr}
};
TokenType.__constructs__ = [TokenType.Datablock,TokenType.Singleton,TokenType.Package,TokenType.Function,TokenType.Assert,TokenType.If,TokenType.Else,TokenType.Switch,TokenType.Case,TokenType.Return,TokenType.Break,TokenType.New,TokenType.Do,TokenType.While,TokenType.For,TokenType.Foreach,TokenType.ForeachStr,TokenType.True,TokenType.False,TokenType.Default,TokenType.Plus,TokenType.Minus,TokenType.Multiply,TokenType.Divide,TokenType.Modulus,TokenType.Assign,TokenType.PlusAssign,TokenType.MinusAssign,TokenType.MultiplyAssign,TokenType.OrAssign,TokenType.AndAssign,TokenType.XorAssign,TokenType.ModulusAssign,TokenType.DivideAssign,TokenType.ShiftLeftAssign,TokenType.ShiftRightAssign,TokenType.LessThan,TokenType.GreaterThan,TokenType.LessThanEqual,TokenType.GreaterThanEqual,TokenType.Not,TokenType.NotEqual,TokenType.Equal,TokenType.Tilde,TokenType.StringEquals,TokenType.StringNotEquals,TokenType.Concat,TokenType.SpaceConcat,TokenType.TabConcat,TokenType.NewlineConcat,TokenType.Continue,TokenType.LogicalAnd,TokenType.LogicalOr,TokenType.LeftBitShift,TokenType.RightBitShift,TokenType.BitwiseAnd,TokenType.BitwiseOr,TokenType.BitwiseXor,TokenType.Label,TokenType.Int,TokenType.String,TokenType.TaggedString,TokenType.Exp,TokenType.Float,TokenType.Ws,TokenType.LParen,TokenType.Colon,TokenType.RParen,TokenType.LBracket,TokenType.RBracket,TokenType.DoubleColon,TokenType.Comma,TokenType.Semicolon,TokenType.LeftSquareBracket,TokenType.RightSquareBracket,TokenType.Or,TokenType.In,TokenType.Dollar,TokenType.Dot,TokenType.IntName,TokenType.IntNameR,TokenType.PlusPlus,TokenType.MinusMinus,TokenType.QuestionMark,TokenType.Eof,TokenType.Comment,TokenType.Unknown];
class Type {
...
class expr_ParenthesisExpr extends expr_Expr {
	constructor(expr) {
		...
		this._hx_constructor(expr);
	}
	_hx_constructor(expr) {
		this.implicit = false;
...
class expr_AssertStmt extends expr_Stmt {
	constructor(lineNo,expr,message) {
		super(lineNo);
		this.expr = expr;
		this.message = message;
	}
	print(indent,isStmt) {
		return this.printIndent(indent) + ("assert(" + this.expr.print(indent,false) + (this.message == null ? "" : ", " + this.message.lexeme) + ");\n");
	}
	visitStmt(optimizerPass) {
		this.expr.visitStmt(optimizerPass);
		optimizerPass.visitAssertStmt(this);
	}
}
expr_AssertStmt.__name__ = true;
expr_AssertStmt.__super__ = expr_Stmt;
Object.assign(expr_AssertStmt.prototype, {
	__class__: expr_AssertStmt
});
class expr_IfStmt extends expr_Stmt {
	...
	print(indent,isStmt) {
		...
		let bodyPart = expr_Stmt.printBlock(this.body,indent + 1) + (this.body.length > 1 || this.ifStmtList ? this.printIndent(indent) + "}" : "");
		if(this.elseBlock != null) {
			...
			if(this.elseBlock != null) {
				let elsePart = expr_Stmt.printBlock(this.elseBlock,indent + 1) + (this.elseBlock.length > 1 || this.elseStmtList ? this.printIndent(indent) + "}\n" : "");
				bodyPart += elsePart;
			}
		} else if(this.ifStmtList) {
			bodyPart += "\n";
		}
...
class expr_LoopStmt extends expr_Stmt {
	...
	print(indent,isStmt) {
		if(this.isForLoop) {
			...
			let bodyPart = expr_Stmt.printBlock(this.body,indent + 1) + (this.body.length > 1 || this.isStatementList ? this.printIndent(indent) + "}\n" : "");
...
class expr_IterStmt extends expr_Stmt {
	constructor(lineNo,init,collection,body) {
		super(lineNo);
		this.init = init;
	...
	visitStmt(optimizerPass) {
		this.init.visitStmt(optimizerPass);
		this.collection.visitStmt(optimizerPass);
		expr_Stmt.visitBlock(optimizerPass,this.body);
		optimizerPass.visitIterStmt(this);
	}
}
expr_IterStmt.__name__ = true;
expr_IterStmt.__super__ = expr_Stmt;
Object.assign(expr_IterStmt.prototype, {
	__class__: expr_IterStmt
});
...
class expr_CommaCatExpr extends expr_BinaryExpr {
	...
	compile(compiler,context,typeReq) {
		...
		if(typeReq == expr_TypeReq.ReqInt || typeReq == expr_TypeReq.ReqFloat) {
			console.log("src/expr/Expr.hx:979:","Warning: Converting comma string to number");
		}
		if(typeReq == expr_TypeReq.ReqInt) {
			context.codeStream[context.ip++] = 56;
...
class expr_InternalNameAccessExpr extends expr_Expr {
	constructor(expr,nameExpr) {
		super(expr.lineNo);
		this.expr = expr;
	...
	visitStmt(optimizerPass) {
		this.expr.visitStmt(optimizerPass);
		this.nameExpr.visitStmt(optimizerPass);
		optimizerPass.visitInternalNameAccessExpr(this);
	}
}
expr_InternalNameAccessExpr.__name__ = true;
expr_InternalNameAccessExpr.__super__ = expr_Expr;
Object.assign(expr_InternalNameAccessExpr.prototype, {
	__class__: expr_InternalNameAccessExpr
});
